...
func getTestConfig() *testConfig {
	...
	spaceID := os.Getenv("PHOENIX_SPACE_ID")
	endpoint := os.Getenv("PHOENIX_COLLECTOR_ENDPOINT")
	if endpoint == "" {
		// Default to Phoenix Cloud when SpaceID is set, otherwise localhost
		if spaceID != "" {
...
// skipIfNoAPIKey skips the test if PHOENIX_API_KEY is not set.
func skipIfNoAPIKey(t *testing.T) *testConfig {
	t.Helper()
	cfg := getTestConfig()
	if cfg == nil {
		t.Skip("PHOENIX_API_KEY not set, skipping integration test")
	}
	return cfg
}

// isAuthError checks if an error is likely an authentication/authorization error.
...
// skipOnAuthError logs the error and skips the test if it's an auth error.
func skipOnAuthError(t *testing.T, err error, operation string) {
	t.Helper()
	if isAuthError(err) {
		t.Skipf("%s failed with auth error (API key may be invalid or expired): %v", operation, err)
	}
}
...
func openTestProvider(t *testing.T, cfg *testConfig, projectName string) llmops.Provider {
	...
	opts := []llmops.ClientOption{
		...
		llmops.WithEndpoint(cfg.Endpoint),
		llmops.WithProjectName(projectName),
	}
	// Use Workspace for Phoenix SpaceID
	if cfg.SpaceID != "" {
		opts = append(opts, llmops.WithWorkspace(cfg.SpaceID))
	}

	provider, err := llmops.Open("phoenix", opts...)
	if err != nil {
...
func TestProviderRegistration(t *testing.T) {
	...
	if !found {
		t.Error("phoenix provider not registered")
	}
}

func TestProviderInfo(t *testing.T) {
	info, ok := llmops.GetProviderInfo("phoenix")
	if !ok {
		t.Fatal("phoenix provider info not found")
	}

	if info.Name != "phoenix" {
		t.Errorf("expected name 'phoenix', got %q", info.Name)
	}
	if info.Description == "" {
...
func TestProviderName(t *testing.T) {
	...
	if provider.Name() != "phoenix" {
		t.Errorf("expected provider name 'phoenix', got %q", provider.Name())
	}
}
...
func TestSpanTypes(t *testing.T) {
	...
	spanTypes := []llmops.SpanType{
		...
		llmops.SpanTypeLLM,
		llmops.SpanTypeTool,
		llmops.SpanTypeRetrieval,
		llmops.SpanTypeAgent,
		llmops.SpanTypeChain,
	}
...
func TestCreateProject(t *testing.T) {
	...
	project, err := provider.CreateProject(ctx, projectName,
		llmops.WithProjectDescription("Test project for integration tests"),
	)
	if err != nil {
	...
	if project.ID == "" {
		t.Logf("Note: project ID is empty (this may be expected for some Phoenix versions)")
	}
}
...
func TestListProjects(t *testing.T) {
	...
	if err != nil {
		t.Logf("ListProjects returned error: %v", err)
		return
	}
...
func TestCreateDataset(t *testing.T) {
	...
	if err != nil {
		skipOnAuthError(t, err, "CreateDataset")
...
func TestAddDatasetItems(t *testing.T) {
	...
	if err != nil {
		t.Logf("CreateDataset returned error: %v", err)
		return
	}
	...
	if err := provider.AddDatasetItems(ctx, datasetName, items); err != nil {
		t.Logf("AddDatasetItems returned error: %v", err)
	}
}
...
func TestListDatasets(t *testing.T) {
	...
	if err != nil {
		t.Logf("ListDatasets returned error: %v", err)
		return
	}
...
func TestGetDatasetByID(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	provider := openTestProvider(t, cfg, "test-get-dataset-by-id")
	ctx := context.Background()

	// First create a dataset
	datasetName := fmt.Sprintf("test-get-dataset-%d", time.Now().UnixNano())
	created, err := provider.CreateDataset(ctx, datasetName)
	if err != nil {
		t.Fatalf("failed to create dataset: %v", err)
	}

	// Get it by ID
	dataset, err := provider.GetDatasetByID(ctx, created.ID)
	if err != nil {
		t.Fatalf("failed to get dataset by ID: %v", err)
	}

	if dataset.ID != created.ID {
		t.Errorf("expected dataset ID %q, got %q", created.ID, dataset.ID)
	}
	if dataset.Name != datasetName {
		t.Errorf("expected dataset name %q, got %q", datasetName, dataset.Name)
	}
...
func TestDeleteDataset(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	provider := openTestProvider(t, cfg, "test-delete-dataset")
	ctx := context.Background()

	// First create a dataset
	datasetName := fmt.Sprintf("test-delete-dataset-%d", time.Now().UnixNano())
	created, err := provider.CreateDataset(ctx, datasetName)
	if err != nil {
		t.Fatalf("failed to create dataset: %v", err)
	}
	...
	// Delete it
	if err := provider.DeleteDataset(ctx, created.ID); err != nil {
		t.Fatalf("failed to delete dataset: %v", err)
	}

	// Verify it's gone
	_, err = provider.GetDatasetByID(ctx, created.ID)
	if err == nil {
...
func TestGetPromptByTag(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	provider := openTestProvider(t, cfg, "test-get-prompt-tag")
	ctx := context.Background()

	promptName := fmt.Sprintf("test-prompt-tag-%d", time.Now().UnixNano())
	template := "Tagged prompt: {{input}}"
	...
	// Try to get by tag - this may not work if tags aren't set via API
	_, err = provider.GetPrompt(ctx, promptName, "production")
	if err != nil {
		t.Logf("GetPrompt with tag returned error: %v (expected if tag not set)", err)
	}
}
...
func TestListPrompts(t *testing.T) {
	...
	if err != nil {
		t.Logf("ListPrompts returned error: %v", err)
		return
	}
...
func TestCreateAnnotation(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	provider := openTestProvider(t, cfg, "test-create-annotation")
	ctx := context.Background()

	// First create a trace to annotate
	ctx, trace, err := provider.StartTrace(ctx, "annotation-test-trace")
	if err != nil {
		t.Fatalf("failed to start trace: %v", err)
	}

	ctx, span, err := provider.StartSpan(ctx, "annotation-test-span")
	if err != nil {
		t.Fatalf("failed to start span: %v", err)
	}
	...
	// Create an annotation on the span
	annotation := llmops.Annotation{
		SpanID:      span.ID(),
		Name:        "test-annotation",
		Score:       0.85,
		Label:       "good",
		Explanation: "Test annotation for integration tests",
		Source:      llmops.AnnotatorKindCode,
	}

	if err := provider.CreateAnnotation(ctx, annotation); err != nil {
		t.Errorf("failed to create annotation: %v", err)
	}
}

func TestListAnnotations(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	provider := openTestProvider(t, cfg, "test-list-annotations")
	ctx := context.Background()

	// First create a trace with annotation
	ctx, trace, err := provider.StartTrace(ctx, "list-annotation-trace")
	if err != nil {
		t.Fatalf("failed to start trace: %v", err)
	}

	ctx, span, err := provider.StartSpan(ctx, "list-annotation-span")
	if err != nil {
		t.Fatalf("failed to start span: %v", err)
	}
	...
	annotation := llmops.Annotation{
		SpanID: spanID,
		Name:   "list-test-annotation",
		Score:  0.9,
		Source: llmops.AnnotatorKindCode,
	}

	if err := provider.CreateAnnotation(ctx, annotation); err != nil {
		t.Logf("CreateAnnotation returned error: %v", err)
		return
	}

	// List annotations for the span
	annotations, err := provider.ListAnnotations(ctx, llmops.ListAnnotationsOptions{
		SpanIDs: []string{spanID},
	})
	if err != nil {
		t.Logf("ListAnnotations returned error: %v", err)
		return
	}

	t.Logf("Found %d annotations for span %s", len(annotations), spanID)
	for _, ann := range annotations {
		t.Logf("  Annotation: %s (Score: %.2f)", ann.Name, ann.Score)
	}
}
...
func TestFullWorkflow(t *testing.T) {
	...
	// 2. Create a retrieval span
	ctx, retrieverSpan, err := provider.StartSpan(ctx, "retrieval",
		llmops.WithSpanType(llmops.SpanTypeRetrieval),
		llmops.WithSpanInput(map[string]any{"query": "relevant documents"}),
	)
	if err != nil {
		t.Fatalf("failed to start retriever span: %v", err)
	}

	if err := retrieverSpan.SetOutput(map[string]any{
		"documents": []string{"doc1", "doc2"},
	}); err != nil {
		t.Errorf("failed to set retriever output: %v", err)
	}
	if err := retrieverSpan.End(); err != nil {
		t.Errorf("failed to end retriever span: %v", err)
	}
	t.Logf("Created retriever span: %s", retrieverSpan.ID())
...
// =============================================================================
// Verification Tests: Read Back From Phoenix
// =============================================================================

func TestVerifyTraceInPhoenix(t *testing.T) {
	cfg := skipIfNoAPIKey(t)
	...
	// Use the Phoenix SDK directly to verify spans exist
	phoenixOpts := []phoenix.Option{
		phoenix.WithAPIKey(cfg.APIKey),
	}
	if cfg.Endpoint != "" {
		phoenixOpts = append(phoenixOpts, phoenix.WithURL(cfg.Endpoint))
	}
	if cfg.SpaceID != "" {
		phoenixOpts = append(phoenixOpts, phoenix.WithSpaceID(cfg.SpaceID))
	}

	client, err := phoenix.NewClient(phoenixOpts...)
	if err != nil {
		t.Fatalf("failed to create Phoenix client for verification: %v", err)
	}

	// Try to get spans for the project
	spans, _, err := client.GetSpans(ctx, "test-verify-trace")
	if err != nil {
		t.Logf("Note: GetSpans returned error: %v (may need time to propagate)", err)
		return
	}

	t.Logf("Found %d spans in project", len(spans))
	for _, s := range spans {
		t.Logf("  Span: %s (TraceID: %s)", s.Name, s.TraceID)
	}
}
